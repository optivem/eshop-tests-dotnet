...
public class PlaceOrderPositiveApiTest : BaseE2eTest
{
    ...
    public async Task ShouldPlaceOrderWithCorrectSubtotalPrice()
    {
        ...
        var placeOrderJson = $$"""{"sku":"{{sku}}","quantity":"5","country":"{{Defaults.COUNTRY}}"}""";
        var orderNumber = await PlaceOrderViaApiAsync(placeOrderJson);

        var subtotal = await ViewOrderSubtotalViaApiAsync(orderNumber);
        subtotal.ShouldBe(100.00m);
    }
    ...
    public async Task ShouldPlaceOrderWithCorrectSubtotalPriceParameterized(string unitPrice, string quantity, string expectedSubtotalPrice)
    {
        ...
        var placeOrderJson = $$"""{"sku":"{{sku}}","quantity":"{{quantity}}","country":"{{Defaults.COUNTRY}}"}""";
        var orderNumber = await PlaceOrderViaApiAsync(placeOrderJson);

        var subtotal = await ViewOrderSubtotalViaApiAsync(orderNumber);
        subtotal.ShouldBe(decimal.Parse(expectedSubtotalPrice));
    }
    ...
    public async Task ShouldPlaceOrder()
    {
        ...
        order.GetProperty("country").GetString().ShouldBe(Defaults.COUNTRY);
        order.GetProperty("quantity").GetInt32().ShouldBe(5);
        order.GetProperty("unitPrice").GetDecimal().ShouldBe(20.00m);
        order.GetProperty("subtotalPrice").GetDecimal().ShouldBe(100.00m);
        order.GetProperty("status").GetString().ShouldBe("PLACED");
        order.GetProperty("discountRate").GetDecimal().ShouldBeGreaterThanOrEqualTo(0);
        order.GetProperty("discountAmount").GetDecimal().ShouldBeGreaterThanOrEqualTo(0);
        order.GetProperty("taxRate").GetDecimal().ShouldBeGreaterThanOrEqualTo(0);
        order.GetProperty("taxAmount").GetDecimal().ShouldBeGreaterThanOrEqualTo(0);
    ...
    private async Task<decimal> ViewOrderSubtotalViaApiAsync(string orderNumber)
    {
        var uri = new Uri(_configuration.ShopApiBaseUrl + $"/api/orders/{orderNumber}");
        var response = await _shopApiHttpClient!.GetAsync(uri);
        ((int)response.StatusCode).ShouldBe((int)HttpStatusCode.OK);
        var body = await response.Content.ReadAsStringAsync();
        using var doc = JsonDocument.Parse(body);
        return doc.RootElement.GetProperty("subtotalPrice").GetDecimal();
    }
