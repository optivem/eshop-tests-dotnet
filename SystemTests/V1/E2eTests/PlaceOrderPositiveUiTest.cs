...
using SystemTests.V1.E2eTests.Base;
using Shouldly;
using Xunit;

namespace SystemTests.V1.E2eTests;
...
public class PlaceOrderPositiveUiTest : BaseE2eTest
{
    ...
    public async Task ShouldPlaceOrderWithCorrectSubtotalPrice()
    {
        ...
        var subtotalText = await shopUiPage.Locator("[aria-label='Display Subtotal Price']").TextContentAsync();
        var subtotalValue = decimal.Parse((subtotalText ?? "").Replace("$", ""));
        subtotalValue.ShouldBe(100.00m);
    }
    ...
    public async Task ShouldPlaceOrderWithCorrectSubtotalPriceParameterized(string unitPrice, string quantity, string expectedSubtotalPrice)
    {
        ...
        var subtotalText = await shopUiPage.Locator("[aria-label='Display Subtotal Price']").TextContentAsync();
        var subtotalValue = decimal.Parse((subtotalText ?? "").Replace("$", ""));
        subtotalValue.ShouldBe(decimal.Parse(expectedSubtotalPrice));
    }
    ...
    public async Task ShouldPlaceOrder()
    {
        ...
        (await shopUiPage.Locator("[aria-label='Display Country']").TextContentAsync()).ShouldBe(Defaults.COUNTRY);
        ...
        decimal.Parse((await shopUiPage.Locator("[aria-label='Display Subtotal Price']").TextContentAsync() ?? "").Replace("$", "")).ShouldBe(100.00m);
        (await shopUiPage.Locator("[aria-label='Display Status']").TextContentAsync()).ShouldBe("PLACED");
        decimal.Parse((await shopUiPage.Locator("[aria-label='Display Discount Rate']").TextContentAsync() ?? "").Replace("%", "")).ShouldBeGreaterThanOrEqualTo(0);
        decimal.Parse((await shopUiPage.Locator("[aria-label='Display Discount Amount']").TextContentAsync() ?? "").Replace("$", "")).ShouldBeGreaterThanOrEqualTo(0);
        decimal.Parse((await shopUiPage.Locator("[aria-label='Display Tax Rate']").TextContentAsync() ?? "").Replace("%", "")).ShouldBeGreaterThanOrEqualTo(0);
        decimal.Parse((await shopUiPage.Locator("[aria-label='Display Tax Amount']").TextContentAsync() ?? "").Replace("$", "")).ShouldBeGreaterThanOrEqualTo(0);
        decimal.Parse((await shopUiPage.Locator("[aria-label='Display Total Price']").TextContentAsync() ?? "").Replace("$", "")).ShouldBeGreaterThan(0);
    }
